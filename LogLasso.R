...
regll = function(X, y, beta, lambda, tol = 1e-5, maxiter = 1000) {
  ### Parameters:
  ...
  while (diff.obj > tol && obj < Inf) {
    print(paste(j, diff.obj))
    j = j + 1
    prev.obj = obj
    prev.beta = beta
...
lambda.cv = function(lambdas, start, data, resp, k = 5) {
  ...
  for (l in lambdas) {
    ...
    for (k in 1:nrow(folds)) {
      ...
      fit = regll(X = train.X, y = train.y, beta = start, lambda = l)
...
lambda.cv.par = function(lambdas, start, data, resp, k = 5) {
  ...
  res = foreach(i = 1:length(lambdas), .combine = rbind) %dopar% { 
    ...
    for (k in 1:nrow(folds)) {
      ...
      # Perform the logistic-LASSO
      fit = regll(X = train.X, y = train.y, beta = start, lambda = lambdas[i])
      u = exp(as.matrix(test.X) %*% fit$coefficients)
      z = u / (1 + u)
...
# Original implementation in Project #2
LogLASSO = function(X, y, beta, lambda, tol = 1e-10, maxiter = 1000) {
  ### Parameters:
  # X : design matrix, does include an intercept column                                                 
  # y : response variable (should be binary)                          
  # beta : starting beta coefficients to start from                   
